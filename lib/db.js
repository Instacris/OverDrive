/* =========================================================
   OVERDRIVE — Conexión a la base de datos en la nube
   La usan las funciones de /api/ cuando el sitio corre en Vercel.
   Guarda TODO el estado (productos + mensajes) como un único
   objeto JSON en Redis Cloud (integración de Vercel Storage).
   ========================================================= */
const crypto = require('crypto');
const Redis = require('ioredis');
const { PRODUCTOS_INICIALES } = require('../js/catalogo-inicial.js');

/* Vercel inyecta esta variable al conectar la base de datos
   (integración "Redis" / Redis Cloud, pestaña Storage). */
const REDIS_URL = process.env.REDIS_URL;

if (!REDIS_URL) {
  throw new Error(
    'Falta la base de datos: conecta un almacén Redis en Vercel ' +
    '(pestaña Storage) y vuelve a desplegar.'
  );
}

const CLAVE_DB = 'overdrive:db';

/* Contraseña del panel. En Vercel se define como variable de
   entorno CLAVE_ADMIN; si no, usa este valor por defecto. */
const CLAVE_ADMIN = process.env.CLAVE_ADMIN || 'fantasma123';

/* Reutilizamos la misma conexión entre invocaciones de una función
   "tibia" (evita abrir una conexión nueva en cada petición, lo que
   agotaría rápido el límite de conexiones del plan gratuito). */
let cliente = null;
function obtenerCliente() {
  if (!cliente) {
    cliente = new Redis(REDIS_URL, {
      maxRetriesPerRequest: 3,
      connectTimeout: 8000
    });
    cliente.on('error', () => {}); // Evita que un error de red tumbe el proceso
  }
  return cliente;
}

async function leerBD() {
  const redis = obtenerCliente();
  const crudo = await redis.get(CLAVE_DB);

  let datos = null;
  try {
    datos = crudo ? JSON.parse(crudo) : null;
  } catch { /* datos corruptos: se regeneran abajo */ }

  if (!datos || !Array.isArray(datos.productos)) {
    datos = { productos: PRODUCTOS_INICIALES, mensajes: [] };
    await redis.set(CLAVE_DB, JSON.stringify(datos));
  }
  return datos;
}

async function guardarBD(datos) {
  const redis = obtenerCliente();
  await redis.set(CLAVE_DB, JSON.stringify(datos));
}

/* Normaliza el cuerpo de la petición: Vercel a veces lo entrega como
   objeto ya interpretado y a veces como texto (si falta Content-Type).
   Devuelve siempre el objeto/array, o null si no se pudo interpretar. */
function cuerpoJSON(req) {
  let cuerpo = req.body;
  if (Buffer.isBuffer(cuerpo)) cuerpo = cuerpo.toString('utf8');
  if (typeof cuerpo === 'string') {
    try { return JSON.parse(cuerpo); } catch { return null; }
  }
  return cuerpo || null;
}

/* ---------- Capas de seguridad ---------- */

/* Compara la contraseña en tiempo constante: evita que un atacante
   deduzca la clave midiendo cuánto tarda la respuesta. */
function claveValida(entrada) {
  const a = Buffer.from(String(entrada ?? ''));
  const b = Buffer.from(CLAVE_ADMIN);
  return a.length === b.length && crypto.timingSafeEqual(a, b);
}

/* IP real del visitante (Vercel la entrega en x-forwarded-for). */
function ipDe(req) {
  return (req.headers['x-forwarded-for'] || '').split(',')[0].trim() || 'desconocida';
}

/* Límite de intentos por IP, guardado en Redis.
   - demasiadosEventos: ¿esta IP ya superó el máximo permitido?
   - registrarEvento: anota un intento (la primera vez fija la ventana
     de tiempo; al expirar, el contador se borra solo). */
async function demasiadosEventos(tipo, ip, maximo) {
  const redis = obtenerCliente();
  const n = Number(await redis.get(`overdrive:rl:${tipo}:${ip}`)) || 0;
  return n >= maximo;
}

async function registrarEvento(tipo, ip, ventanaSegundos) {
  const redis = obtenerCliente();
  const clave = `overdrive:rl:${tipo}:${ip}`;
  const n = await redis.incr(clave);
  if (n === 1) await redis.expire(clave, ventanaSegundos);
}

module.exports = {
  leerBD, guardarBD, CLAVE_ADMIN, cuerpoJSON,
  claveValida, ipDe, demasiadosEventos, registrarEvento
};
